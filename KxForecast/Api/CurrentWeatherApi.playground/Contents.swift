...
//        fatalError("URL 생성 실패")     //공부할때만 사용해야하고 출시할때는 삭제해야함
...
func fetchCurrentWeather(cityName: String, completion: @escaping (Result<CurrentWeather, Error>) -> ()) {
    let urlStr = "http://api.openweathermap.org/data/2.5/weather?q=\(cityName)&appid=93e0788f8e0bd92f4ea11a310a22623a&units=metric&lang=kr"
...
func fetchCurrentWeather(cityId: Int, completion: @escaping (Result<CurrentWeather, Error>) -> ()) {
    let urlStr = "http://api.openweathermap.org/data/2.5/weather?id=\(cityId)&appid=93e0788f8e0bd92f4ea11a310a22623a&units=metric&lang=kr"
...
func fetchCurrentWeather(location: CLLocation, completion: @escaping (Result<CurrentWeather, Error>) -> ()) {
    let urlStr = "http://api.openweathermap.org/data/2.5/weather?lat=\(location.coordinate.latitude)&lon=\(location.coordinate.longitude)&appid=93e0788f8e0bd92f4ea11a310a22623a&units=metric&lang=kr"
